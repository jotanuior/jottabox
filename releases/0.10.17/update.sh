...
backup_file "$PS3_FW" "jottabox-rpcs3-firmware"
...
echo ">>> Instalando gerenciador de firmware RPCS3"

curl -fsSL \
  "$RAW/scripts/jottabox-rpcs3-firmware.sh" \
  -o "$TMP/jottabox-rpcs3-firmware"

bash -n "$TMP/jottabox-rpcs3-firmware"

install -m 0755 \
  "$TMP/jottabox-rpcs3-firmware" \
  "$PS3_FW"
...
FW="$HOME/.local/bin/jottabox-rpcs3-firmware"

if [[ ! -x "$FW" ]]; then
    echo "ERRO: gerenciador de firmware RPCS3 não encontrado."
    exit 1
fi

if ! find \
    "$HOME/.var/app/net.rpcs3.RPCS3/config/rpcs3/dev_flash/data/font" \
    -maxdepth 1 \
    -type f \
